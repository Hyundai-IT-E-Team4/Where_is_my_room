   ...
   if(valueCheck(content)){
      $.ajax({
         ...
         if(userNickname != writerNickName){
            let sendData = {"command" : "댓글", "receiverName" : writerNickName, "content" : `<a href="/team4/boards/${getBoardNumber()}">새로운 댓글을 달았습니다.</a>`}
   ...
   if(valueCheck(content)){
      $.ajax({
         ...
         if(userNickname != commentWriterName){
            let sendData = {"command" : "대댓글", "receiverName" : commentWriterName, "content" : `<a href="/team4/boards/${getBoardNumber()}">새로운 대댓글을 달았습니다.</a>`}
            send(JSON.stringify(sendData));
